...
import { bookings, getUserById, getServiceById, getVehicleById, formatDateTime, formatPrice } from "@/lib/data"
                ...
                return (
                  <tr key={booking.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-primary">{formatPrice(service?.basePrice ?? 0)}</p>
                    </td>
                    <td className="px-4 py-3 text-sm text-muted-foreground">
                      {formatDateTime(booking.scheduledAt)}
                    </td>
                    <td className="px-4 py-3">
                      <span className={cn("rounded-full px-2 py-0.5 text-xs font-medium", statusColors[booking.status])}>
                        {booking.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
          ...
          {filtered.map((booking) => {
            ...
            return (
              <div key={booking.id} className="rounded-md border border-border bg-background p-4">
                ...
                <p className="mt-2 text-xs text-muted-foreground">{formatDateTime(booking.scheduledAt)}</p>
                <div className="mt-3">
                  <Select
                    value={booking.status}
                    onValueChange={(v) => updateStatus(booking.id, v as BookingStatus)}
